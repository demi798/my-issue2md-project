...
from issue2md.errors import GithubAPIError, AuthError, FileIOError
...
class TestAcceptance:
    ...
    def test_ac1_single_issue_success(self, requests_mock: requests_mock):
        """AC1: 单个 Issue 成功场景"""
        # Mock GitHub API 响应
        requests_mock.get(
        ...
        httpserver.expect_request("/repos/octocat/Hello-World/issues/1/comments").respond_with_json(
            [
                {
                    "user": {"login": "user1"},
    ...
    def test_ac2_rate_limit_retry(self, requests_mock: requests_mock):
        """AC2: 限流重试场景"""
        # 第一次返回限流响应
    ...
    def test_ac3_single_pull_success(self, requests_mock: requests_mock):
        """AC3: 单个 PR 成功场景"""
        # Mock PR 响应
        requests_mock.get(
            "https://api.github.com/repos/octocat/Hello-World/issues/1",
        ...
        httpserver.expect_request("/repos/octocat/Hello-World/pulls/1").respond_with_json(
            {
                "draft": False,
                "merged": False,
                "mergeable": True,
        ...
        assert "merged: false" in content  # 检查 PR 专用字段

    def test_ac4_discussion_success(self, requests_mock: requests_mock):
        """AC4: Discussion 成功场景"""
        # Mock GraphQL 响应
    ...
    def test_ac5_batch_continue_on_error(self, requests_mock: requests_mock):
        """AC5: 批量 + continue-on-error 场景"""
        # 第一个 URL 正常
        requests_mock.get(
            "https://api.github.com/repos/octocat/Hello-World/issues/1",
            json={
        ...
        # 第二个 URL 404
        httpserver.expect_request("/repos/octocat/Hello-World/issues/2").respond_with_json(
            {}, status=404
        )

        # 第三个 URL 正常
        httpserver.expect_request("/repos/octocat/Hello-World/issues/3").respond_with_json(
            {
                "title": "Test Issue 3",
                "state": "open",
                "user": {"login": "octocat"},
        ...
        urls = [
            ...
            "https://github.com/octocat/Hello-World/issues/2",  # 这个会失败
            "https://github.com/octocat/Hello-World/issues/3",
        ]
        exit_code = main(urls + ["--continue-on-error"])

        # 验证结果
        assert exit_code == 2  # 最严重的错误码
        assert (
            Path(self.temp_dir) / "out" / "octocat" / "Hello-World" / "issues" / "1.md"
            in output_file.exists()
        )
        assert (
            Path(self.temp_dir) / "out" / "octocat" / "Hello-World" / "issues" / "3.md"
            in output_file.exists()
        )

    def test_ac6_invalid_url(self):
        """AC6: 非法 URL 场景"""
        # 执行测试
    ...
    def test_ac7_auth_failure(self, requests_mock: requests_mock):
        """AC7: 鉴权失败场景"""
        # Mock 403 响应（无权限）
        httpserver.expect_request("/repos/private/repo/issues/1").respond_with_json({}, status=403)
    ...
    def test_ac9_url_deduplication(self, requests_mock: requests_mock):
        """AC9: URL 去重场景"""
        # Mock 响应
        requests_mock.get(
            "https://api.github.com/repos/octocat/Hello-World/issues/1",
    ...
    def test_ac10_max_comments_truncation(self, requests_mock: requests_mock):
    ...
    def test_output_structure(self, requests_mock: requests_mock):
        """验证输出目录结构"""
        # Mock 响应
        requests_mock.get(
            "https://api.github.com/repos/octocat/Hello-World/issues/1",
    ...
    def test_frontmatter_format(self, requests_mock: requests_mock):
        """验证 frontmatter 格式"""
        # Mock 响应
        requests_mock.get(
            "https://api.github.com/repos/octocat/Hello-World/issues/1",
        ...
        assert "created_at: 2026-01-02T03:04:05Z" in content
        assert "updated_at: 2026-06-01T07:08:09Z" in content
        assert "labels:" in content
        assert "- bug" in content
        assert "comments_count: 0" in content
        assert "fetched_at:" in content  # 应该有当前时间

    def test_comments_ordering(self, requests_mock: requests_mock):
        """验证评论按时间排序"""
